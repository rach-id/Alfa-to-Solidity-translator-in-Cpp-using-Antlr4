...
class Listener : public ALFABaseListener {
	...
	Listener(ALFAParser* p, string f) : outputFile(f, ios::out), boolCount(0), oldContractsCount(0), newContractsCount(0), namespaceCount(0), translateAttribute(false), inModifier(false), ruleCount(0) {
...
public:	
	...
	/* ---------------------------------------------------------------------------------------------------------------------------*/
		...
		*/
		...
		if(!((ctx->clauseDefinition())->booleenExpression()).empty()) {
			for( auto booleen : (ctx->clauseDefinition())->booleenExpression() ) {
				if (booleen->TARGETRESSOURCE()) {
					string target = (booleen->TARGETRESSOURCE())->toString() ;
					cout << "\n\n\n\n\n\n" << target << "\n\n\n\n\n" << endl;
	...
	/* ---------------------------------------------------------------------------------------------------------------------------*/
  	...
  	virtual void enterClauseDefinition(ALFAParser::ClauseDefinitionContext *ctx) {
  		if(inModifier) {
  			std::vector<ALFAParser::BooleenExpressionContext *> booleen = ctx->booleenExpression();
			do {
					cout << "1" << endl;
					booleen = booleen[0]->booleenExpression();
			}
			while(!(booleen[0]->booleenExpression()).empty());
  		}
